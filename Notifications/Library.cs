...
namespace AVTemp
{
    ...
    public class Notifs : BaseUnityPlugin
    {
        private void Awake()
        {
        ...
        private void Init()
        {
            ...
            Notifs.NotifiText = this.Testtext;
        }
        ...
        public static void SendNotification(string NotificationText)
        {
            if (!disableNotifs)
            {
                try
                {
                    if (Notifs.IsEnabled && Notifs.PreviousNotifi != NotificationText)
                    {
                        if (!NotificationText.Contains(Environment.NewLine))
                        {
                            NotificationText += Environment.NewLine;
                        }
                        Notifs.NotifiText.text = Notifs.NotifiText.text + NotificationText;
                        Notifs.NotifiText.supportRichText = true;
                        Notifs.PreviousNotifi = NotificationText;
                    }
                }
                catch
                {
        ...
        {
            //Notifs.NotifiText.text = "<color=grey>[</color><color=green>SUCCESS</color><color=grey>]</color> <color=white>Notifications cleared.</color>" + Environment.NewLine;
            Notifs.NotifiText.text = "";
        }

        public static void ClearPastNotifications(int amount)
        {
            string text = "";
            foreach (string text2 in Enumerable.ToArray<string>(Enumerable.Skip<string>(Notifs.NotifiText.text.Split(Environment.NewLine.ToCharArray()), amount)))
            {
                if (text2 != "")
                {
                    text = text + text2 + "\n";
